...
def test_check_entry_breakout(strategy, mock_repo):
    ...
    # Gap Up Case
    candle = pd.Series({"date": "2024-01-02", "open": 101.0, "high": 105.0, "low": 99.0, "close": 104.0})
...
def test_manage_active_trade_tp1_split(strategy, mock_repo):
    ...
    candle = pd.Series({"date": "2024-01-05", "open": 105.0, "high": 112.0, "low": 104.0, "close": 111.0})
...
def test_manage_active_trade_gap_over_tp3(strategy, mock_repo):
    ...
    candle = pd.Series({"date": "2024-01-05", "open": 135.0, "high": 140.0, "low": 134.0, "close": 138.0})
    df_hist = pd.DataFrame([candle])
...
def test_manage_active_trade_tp3_final(strategy, mock_repo):
    ...
    # Candle hits TP3
    candle = pd.Series({"date": "2024-01-10", "open": 120.0, "high": 135.0, "low": 119.0, "close": 132.0})
    df_hist = pd.DataFrame([candle])
