...
def test_check_entry_fills_gap_down_on_next_trading_day(
    ...
    with patch(
        "app.tools.market_holidays.MarketHolidayChecker.is_holiday", return_value=False
    ):
        # Act
        result = strategy.check_entry(
            created_trade_data, current_candle, history, mock_repository
        )
...
def test_check_entry_ignores_same_day_check(
    ...
    candle = create_candle(
        "2026-01-01", 90.0, 100.0, 80.0, 95.0
    )  # Low is way below limit
    ...
    with patch(
        ...
        # Act
        result = strategy.check_entry(
            created_trade_data, candle, history, mock_repository
        )
...
def test_manage_active_trade_exits_on_green_sequence(
    ...
    with patch(
        ...
        result = strategy.manage_active_trade(
            active_trade_data, df_history, mock_repository
        )
