...
@pytest.mark.parametrize(
    "open_price, high, low, expected_fill, expected_reason",
    [
        (
            100.0,
            105.0,
            ...
            100.0,
            "GAP UP",
        ),  # 1. Gap Up / Touch Open (Open >= Trigger) -> Fill Open
        (
            102.0,
            105.0,
            95.0,
            102.0,
            "GAP UP",
        ),  # 2. Real Gap Up (Open > Trigger) -> Fill Open
        (
            95.0,
            105.0,
            92.0,
            100.0,
            "BREAKOUT",
        ),  # 3. Intraday Breakout (High > Trigger, Open < Trigger) -> Fill Trigger
...
def test_check_entry_success_scenarios(
    ...
    result = strategy.check_entry(
        base_trade_data, candle, pd.DataFrame(), mock_repository
    )
...
def test_check_entry_day_one_turnaround(
    strategy: HoldTargetStrategy, mock_repository: MagicMock, base_trade_data: dict
) -> None:
...
@pytest.mark.parametrize(
    "open_price, high, low, stop, target, expected_exit_price, expected_reason",
    [
