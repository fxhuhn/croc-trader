...
def test_entry_standard_fill(
    ...
    df_history = pd.concat(
        [pd.DataFrame([prev_candle]), pd.DataFrame([candle])], 
        ignore_index=True
    )
...
def test_take_profit_priority_over_loc(
    ...
    df_history = create_history(
        ...
        highs=[148.0, 142.0, 150.0],  # Jan 6 High = 148.0 (LOC), Jan 8 High = 150.0 > TP
...
def test_entry_rejected_if_missed_on_next_day(
    ...
    # Check on Jan 02
    candle = create_candle("2026-01-02", 105.0, 110.0, 101.0, 102.0) # Low 101 > Limit 100
    df_history = pd.DataFrame([candle])
...
def test_entry_rejected_on_day_plus_two_even_if_price_hit(
    ...
    lows  = [95.0,  101.0, 95.0]  # Jan 02 Low (101) > Limit (100); Jan 03 Low (95) < Limit (100)
