...
def create_market_data(length=250, price_start=100.0, trend=0.1, symbol="TEST", end_date="2025-01-10"):
...
def test_entry_filter_fails_downtrend(strategy):
    ...
    def to_series(d): return {k: pd.Series([v], index=[index_date]) for k, v in d.items()}
    
    results = strategy._filter_market_state(to_series(current_values), to_series(previous_values))
...
def test_results_are_sorted_by_score(strategy):
    ...
    current_series = {
        ...
        "atr_ratio_3day": pd.Series([-1.2, -2.0, -1.5], index=symbols), # Variable Scores
...
def test_run_historical_date_slicing(strategy, mock_provider, mock_repo):
    ...
    data = {
        "close": pd.DataFrame({symbol: [110, 108, 105, 100, 102, 105, 108, 110, 112, 115]}, index=dates),
        "open":  pd.DataFrame({symbol: [112, 110, 107, 105, 104, 107, 110, 115, 114, 117]}, index=dates), # All Red
        "high":  pd.DataFrame({symbol: [115, 112, 110, 108, 106, 110, 115, 118, 120, 122]}, index=dates),
        "low":   pd.DataFrame({symbol: [108, 105, 102, 98,  100, 102, 105, 108, 110, 112]}, index=dates),
...
def test_run_auto_date_detection(strategy, mock_provider):
    ...
    with patch.object(DipBuyerStrategy, '_calculate_signals', return_value=pd.DataFrame()) as mock_calc:
