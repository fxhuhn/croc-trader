...
def strategy(mock_repo, mock_provider):
    return TurnoverTimingStrategy(
        trade_repository=mock_repo, data_provider=mock_provider
    )
...
@patch(
    "app.services.screener.strategies.turnover_timing.MarketHolidayChecker.is_holiday"
)
def test_ranking_selects_top_4_highest_turnover_in_uptrend(
    ...
    data_mega = create_market_data(
        mega_caps, end_date, turnover_val=500_000_000
    )  # 500M
    data_mid = create_market_data(mid_caps, end_date, turnover_val=100_000_000)  # 100M
    data_down = create_market_data(
        bad_trend, end_date, turnover_val=600_000_000, trend="DOWN"
    )  # 600M (Highest but Down)

    # Merge DataFrames
    merged_data = {}
    for col in ["close", "high", "low", "open", "volume"]:
        merged_data[col] = pd.concat(
            [data_mega[col], data_mid[col], data_down[col]], axis=1
        )
