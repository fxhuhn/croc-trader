...
@pytest.fixture
def strategy(mock_trade_repository, mock_signal_repository, mock_provider, sample_rules):
...
def test_run_execution(strategy, mock_signal_repository, mock_trade_repository):
    ...
    assert kwargs['strategy'] == Strategies.HoldTarget # From rule exit logic (Hold_TP3)
    assert kwargs['stop_loss'] == 105.0 # Entry(115) - Risk(10)
    assert kwargs['target'] == 145.0 # Entry(115) + 3*Risk(30)

def test_run_execution_skipped_no_bucket(strategy, mock_signal_repository, mock_trade_repository):
