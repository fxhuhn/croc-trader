...
def test_screener_engine_run_all_executes_strategies(mock_dependencies, mock_strategy) -> None:
    """Verifies that run_all orchestrates strategy execution correctly."""
    # Arrange
    trade_repo, signal_repo, data_provider = mock_dependencies
    engine = ScreenerEngine(trade_repo, signal_repo, data_provider, strategies=[mock_strategy])
...
def test_screener_engine_handles_strategy_failure(mock_dependencies, mock_strategy, caplog) -> None:
