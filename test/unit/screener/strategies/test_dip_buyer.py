...
def test_compute_indicators(strategy: DipBuyerStrategy) -> None:
    ...
    with patch("app.tools.indicators.calculate_sma", return_value=closes.copy()):
        with patch("app.tools.indicators.calculate_atr", return_value=pd.DataFrame(1.0, index=index, columns=["AAPL"])):
            with patch("app.tools.indicators.calculate_ibs", return_value=pd.DataFrame(0.5, index=index, columns=["AAPL"])):
                # Act
...
def test_process_signals_creates_trades(strategy: DipBuyerStrategy, mock_trade_repo: MagicMock) -> None:
...
def test_analyze_single_symbol(strategy: DipBuyerStrategy, mock_data_provider: MagicMock) -> None:
...
def test_run_empty_data(strategy: DipBuyerStrategy, mock_data_provider: MagicMock) -> None:
