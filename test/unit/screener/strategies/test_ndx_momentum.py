...
def test_calculate_analysis_last_trading_day_success(strategy, mock_market_data_provider):
    ...
    with patch("app.services.screener.strategies.ndx_momentum.ExchangeSymbol") as mock_exchange:
        mock_inst = MagicMock()
        ...
        for s in symbols:
            df = pd.DataFrame({
                "date": dates,
                "close": np.linspace(100, 150, 300) if s != "QQQ" else np.linspace(200, 250, 300),
...
def test_calculate_analysis_empty_universe(strategy):
    ...
    with patch("app.services.screener.strategies.ndx_momentum.ExchangeSymbol") as mock_ex:
...
def test_calculate_analysis_missing_qqq(strategy, mock_market_data_provider):
    ...
    with patch("app.services.screener.strategies.ndx_momentum.ExchangeSymbol") as mock_ex:
        ...
        mock_market_data_provider.get_batch_history.return_value = {
            "AAPL": pd.DataFrame({"date": [pd.Timestamp(analysis_date)], "close": [100.0], "high": [105.0], "low": [95.0]})
        }
        result = strategy.calculate_analysis(analysis_date=analysis_date)
        assert result["triggered"] is False
...
def test_calculate_analysis_force_run(strategy, mock_market_data_provider):
    ...
    with patch("app.services.screener.strategies.ndx_momentum.ExchangeSymbol") as mock_ex:
        mock_ex.return_value.nasdaq_100 = ["AAPL"]
        dates = pd.date_range(end=analysis_date, periods=300, freq="B")
        data = {"date": dates, "close": [100.0] * 300, "high": [110.0] * 300, "low": [90.0] * 300}
        mock_market_data_provider.get_batch_history.return_value = {"AAPL": pd.DataFrame(data), "QQQ": pd.DataFrame(data)}
        
        result = strategy.calculate_analysis(analysis_date=analysis_date, force_run=True)
        assert result["triggered"] is True
        assert result["is_rebalance_day"] is False
...
def test_run_triggers_trade_creation(strategy, mock_trade_repository):
    ...
    with patch.object(strategy, "calculate_analysis") as mock_calc:
        mock_calc.return_value = {
            ...
            "price_data": {"close": pd.DataFrame({ "AAPL": [100.0] }, index=[analysis_date])},
            "regime_indicators": {"bull": True, "qqq": 100, "qqq_sma": 90, "breadth_fast": 60, "breadth_slow": 50}
        }
...
def test_create_trades_direct_error_handling(strategy, mock_trade_repository):
    ...
    price_data = {"close": pd.DataFrame({ "AAPL": [100.0], "MSFT": [50.0] }, index=[analysis_date])}
    regime = {"bull": True, "qqq": 100, "qqq_sma": 90, "breadth_fast": 60, "breadth_slow": 50}
    ...
    count = strategy._create_trades_direct(symbols, momentum_scores, roc_map, analysis_date, price_data, regime)
    assert count == 1
