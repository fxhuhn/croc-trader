...
def test_check_entry_breakout_fill(strategy, mock_trade_repo):
    ...
    candle = pd.Series({"open": 149.0, "high": 155.0, "low": 148.0, "close": 152.0, "date": pd.Timestamp("2026-02-18")}, name=pd.Timestamp("2026-02-18"))
...
def test_check_entry_day_one_turnaround(strategy, mock_trade_repo):
    ...
    candle = pd.Series({"open": 145.0, "high": 152.0, "low": 144.0, "close": 151.0, "date": pd.Timestamp("2026-02-18")}, name=pd.Timestamp("2026-02-18"))
...
def test_check_entry_expiration(strategy, mock_trade_repo):
    ...
    candle = pd.Series({"high": 149.0, "date": pd.Timestamp("2026-02-18")}, name=pd.Timestamp("2026-02-18"))
    ...
    with patch.object(strategy, "_get_trading_days_post_signal", return_value=6):
        ...
        assert mock_trade_repo.update_trade.call_args[0][1]["exit_reason"] == ExitReason.EXPIRED
