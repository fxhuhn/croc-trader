...
def test_check_entry_gap_down_fill(strategy, mock_trade_repo):
    ...
    # Open 98 (below 100)
    candle = pd.Series(
        {"open": 98.0, "low": 97.0, "date": pd.Timestamp("2026-02-16")},
...
def test_check_entry_limit_hit(strategy, mock_trade_repo):
    ...
    candle = pd.Series(
        {"open": 102.0, "low": 99.0, "date": pd.Timestamp("2026-02-16")},
        name=pd.Timestamp("2026-02-16"),
    )
...
def test_check_entry_missed_window_expiration(strategy, mock_trade_repo):
    ...
    candle = pd.Series(
        {"open": 105.0, "low": 101.0, "date": pd.Timestamp("2026-02-09")},
        name=pd.Timestamp("2026-02-09"),
    )
