...
def test_check_entry_limit_hit_and_green_count(strategy, mock_trade_repo):
    ...
    candle = pd.Series({"open": 99.0, "low": 98.0, "close": 102.0, "date": pd.Timestamp("2026-02-16")}, name=pd.Timestamp("2026-02-16"))
...
def test_manage_active_green_sequence_exit(strategy, mock_trade_repo):
    ...
    candle = pd.Series({"open": 110.0, "date": pd.Timestamp("2026-02-18")}, name=pd.Timestamp("2026-02-18"))
...
def test_manage_active_friday_time_stop(strategy, mock_trade_repo):
    ...
    # Feb 20th is Friday
    candle = pd.Series({"open": 105.0, "close": 102.0, "date": pd.Timestamp("2026-02-20")}, name=pd.Timestamp("2026-02-20"))
