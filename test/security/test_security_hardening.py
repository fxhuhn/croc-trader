...
def test_traderepository_sql_injection_on_symbol():
    ...
    for call in mock_conn.execute.call_args_list:
        args = call[0]
        if len(args) > 0 and isinstance(args[0], str) and "INSERT INTO trades" in args[0]:
...
def test_telegram_bot_does_not_leak_token_on_error(caplog):
    ...
    with patch("requests.post", side_effect=requests.exceptions.RequestException(error_message)):
...
def test_financial_logic_rejects_non_finite_values():
    ...
    with pytest.raises(ValueError, match="Value for entry must be a finite non-negative number"):
        repo.create_trade("AAPL", "Test", 100, float('inf'), 0, 0, {})
        
    with pytest.raises(ValueError, match="Value for stop_loss must be a finite non-negative number"):
