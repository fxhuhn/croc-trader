...
class NDXMomentumScreener(BaseStrategy):
    ...
    def run(
        ...
        logger.info("[%s] Monthly rebalance triggered for %s", self.name, analysis_result["date"])
    ...
    def calculate_analysis(
        ...
        if not force_run and not is_rebalance_day:
            return {"triggered": False, "date": analysis_date, "is_rebalance_day": False}
        ...
        if not nasdaq_100_symbols:
             ...
             return {"triggered": False, "date": analysis_date, "error": "NDX universe empty"}
        ...
        if not full_history_map:
            ...
            return {"triggered": False, "date": analysis_date, "error": "No market data"}
        ...
        if "QQQ" not in pivoted_data["close"].columns:
             ...
             return {"triggered": False, "date": analysis_date, "error": "QQQ data missing"}
        ...
        try:
            ...
            if target_date > pivoted_data["close"].index[-1]:
                ...
                logger.info("[%s] Requested %s but data ends at %s. Using last available.", self.name, analysis_date, effective_date)
            else:
                effective_date = pivoted_data["close"].index.asof(target_date)
            
            if pd.isna(effective_date):
                 logger.warning("[%s] No price data available at or before %s.", self.name, analysis_date)
                 return {"triggered": False, "date": analysis_date, "error": "No price data found"}
                 
            # If not forcing, we still require exact match for rebalance logic
            if not force_run and effective_date != target_date:
                logger.warning("[%s] %s not in data and not forced. Skip.", self.name, analysis_date)
                return {"triggered": False, "date": analysis_date, "error": f"{analysis_date} not in data"}
        ...
        breadth_fast_average = percentage_above_sma100.rolling(window=10).mean().at[effective_date]
        breadth_slow_average = percentage_above_sma100.rolling(window=50).mean().at[effective_date]
        ...
        if target_date_momentum_sum.empty:
            ...
            return {"triggered": False, "date": effective_date.strftime("%Y-%m-%d"), "error": "No momentum scores"}
        ...
        for symbol in symbols:
            try:
                ...
                trade_context = {
                    ...
                    "breadth_regime": "BULL" if (regime_indicators["breadth_fast"] > regime_indicators["breadth_slow"]) else "BEAR",
