...
def run_bridge_scout_backfill(
    ...
    if clear_existing:
        with signals_session.connect() as conn:
            conn.execute(
                "DELETE FROM trade_logs WHERE trade_id IN (SELECT id FROM trades WHERE strategy='bridge_scout')"
            )
            conn.execute("DELETE FROM trades WHERE strategy='bridge_scout'")
            conn.commit()

    screener = BridgeScoutStrategy(trade_repository, data_provider)
    strategy_engine = BridgeScoutTradeStrategy()
