...
class CapacityMonitor:
    ...
    _concurrent_trades: Dict[str, List[Dict[str, object]]] = field(default_factory=lambda: {
        'dip_buyer': [],
        'turnover_0.5': [],
        'turnover_1.0': [],
        'two_percent': [],
    ...
    def update(self, date: datetime, active_trades: Dict[str, List[dict]]):
        ...
        for strategy, trades in active_trades.items():
            ...
            self._concurrent_trades[strategy].append({
                'date': date,
                'count': count
            })
            
        # Track global concurrency
        self._concurrent_trades['global'].append({
            'date': date,
            'count': total_count
        })
...
class DynamicPositionSizer:
    ...
    def calculate_multiplier(
        ...
        """
        utilization = capacity_monitor.get_current_utilization(strategy, current_concurrent_trades)
    ...
    def calculate_position_size(
        ...
        multiplier = self.calculate_multiplier(strategy, current_concurrent_trades, capacity_monitor)
...
class OverflowProtection:
    ...
    def apply_limits(
        ...
        if projected_total_exposure > self.max_total_exposure:
            ...
            available_exposure_room = max(0.0, self.max_total_exposure - current_exposure)
