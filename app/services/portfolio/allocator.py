...
class PortfolioAllocator:
    ...
    def allocate(self, trade: dict) -> AllocationResult:
        ...
        if not strategy_enum:
             logger.warning(f"[{symbol}] Unknown Strategy for Allocation: {raw_strategy}")
             return AllocationResult(0, 0.0, 0.0, "Unknown Strategy")

        # 1. Dip Buyer (Fixed Budget)
        if strategy_enum == Strategies.DipBuyer:
            budget = self._get_budget_for_strategy(Strategies.DipBuyer, BUDGET_DIP_BUYER)
        ...
        if strategy_enum in (Strategies.HoldTarget, Strategies.CrocSetup, Strategies.SplitTarget):
        ...
        if strategy_enum in (Strategies.TurnOverTiming, Strategies.TurnOverTiming_05, Strategies.TurnOverTiming_10):
