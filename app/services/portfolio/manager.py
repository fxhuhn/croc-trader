    ...
    def process_daily_signals(self) -> int:
        ...
        try:
            ...
            for trade in candidates:
                ...
                if current_size > 0:
                    logger.debug(
                        f"[{symbol}] Skipping Allocation (Size already {current_size})"
                    )
                    continue
                ...
                if allocation.size > 0:
                    ...
                    logger.info(
                        f"[{symbol}] Allocated: {allocation.size} shares ({allocation.reason})"
                    )
                    allocated_count += 1
                else:
            ...
            logger.info(
                f"PortfolioManager: Finished. Allocated {allocated_count} new trades."
            )
