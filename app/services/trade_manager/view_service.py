...
class TradeViewService:
    ...
    def prepare_trade_view(self, trade: dict[str, Any]) -> TradeViewData:
        ...
        if trade.get("status") == TradeStatus.ACTIVE and entry_price > 0:
            ...
            for key in [TargetColumn.TARGET_PRICE, TargetColumn.TP3, TargetColumn.TAKE_PROFIT_3, TargetColumn.TP1, TargetColumn.TAKE_PROFIT_1]:
        ...
        view_data: TradeViewData = {
            ...
            "current_stop_loss": float(trade.get("current_stop_loss") or 0.0), # TradeData field name
            "current_target": float(trade.get("current_target") or 0.0), # TradeData field name
            "budget": float(trade.get("budget") or 0.0),
            "signal_context": trade.get("signal_context"),
            "exit_reason": trade.get("exit_reason"),
            
            # Display Fields
            "stop_loss": float(trade.get("current_stop_loss") or 0.0), # Aliased for view
            "take_profit": 0.0,
    ...
    def generate_sparkline(self, dates: list[Any], prices: list[float], is_positive: bool) -> str:
    ...
    def generate_donut_chart(self, labels: list[str], values: list[float], colors: list[str]) -> str:
    ...
    def attach_sparklines(self, trades: list[TradeViewData]) -> None:
        ...
        for trade in trades:
            symbol = trade["symbol"]
            rows = history_dataframe[history_dataframe["symbol"] == symbol].sort_values("date")
    ...
    def get_portfolio_summary(self, active_trades: list[TradeViewData]) -> dict[str, float | int]:
        """Calculates summary metrics for active trades."""
        total_invested = sum(trade["entry_price"] * trade["initial_size"] for trade in active_trades)
    ...
    def get_closed_summary(self, closed_trades: list[TradeViewData]) -> dict[str, float | int]:
    ...
    def group_trades_by_symbol(self, trades: list[TradeViewData]) -> list[dict[str, Any]]:
    ...
    def group_trades_history(self, trades: list[TradeViewData]) -> list[dict[str, Any]]:
        ...
        # Return sorted list
        return sorted(list(grouped.values()), key=lambda x: str(x["max_exit"]), reverse=True)

    def get_index_stats(self, trades: list[TradeViewData]) -> dict[str, dict[str, Any]]:
        """Aggregates PnL statistics by Index (SPX, NDX, etc.)."""
        statistics = {
            IndexAliases.SPX: {"name": IndexAliases.SPX, "count": 0, "win": 0, "loss": 0, "pnl": 0.0},
            IndexAliases.NDX: {"name": IndexAliases.NDX, "count": 0, "win": 0, "loss": 0, "pnl": 0.0},
            IndexAliases.DOW: {"name": IndexAliases.DOW, "count": 0, "win": 0, "loss": 0, "pnl": 0.0},
            IndexAliases.RUS: {"name": IndexAliases.RUS, "count": 0, "win": 0, "loss": 0, "pnl": 0.0},
            IndexAliases.NO_INDEX: {"name": IndexAliases.NO_INDEX, "count": 0, "win": 0, "loss": 0, "pnl": 0.0},
        }
