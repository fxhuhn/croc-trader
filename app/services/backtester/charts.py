    ...
    if not sorted_thresholds:
        ...
        for threshold in sorted_thresholds:
            ...
            if end_segment > current_start:
                track_steps.append(
                    {
                        "range": [current_start, end_segment],
                        "color": track_color,
                    }
                )
        ...
        if current_start < maximum_value:
            track_steps.append(
                {
                    "range": [current_start, maximum_value],
                    "color": track_color,
                }
            )
...
def generate_backtest_charts(
    ...
    html_eq = fig_eq.to_html(
        full_html=False,
        include_plotlyjs=False,
        div_id=f"{id_prefix}-equity-curve-chart",
        config={"displayModeBar": False, "responsive": True},
    )
    html_dd = fig_dd.to_html(
        full_html=False,
        include_plotlyjs=False,
        div_id=f"{id_prefix}-underwater-chart",
        config={"displayModeBar": False, "responsive": True},
    )
...
def generate_win_rate_gauge(value_pct: float) -> str:
    ...
    return _generate_minimal_gauge(
        value_pct, 0, 100, color, "%", thresholds=[35.0, 60.0]
    )
...
def generate_sqn_gauge(value: float) -> str:
    ...
    return _generate_minimal_gauge(
        value, 0, 10.0, classification.color, "", thresholds=[1.0, 2.0, 3.0, 5.0, 7.0]
    )
...
def generate_regime_overlay_chart(daily_df: pd.DataFrame) -> str:
    ...
    if "vix" in dataframe.columns:
        ...
        dataframe["vix"] = (
            pd.to_numeric(dataframe["vix"], errors="coerce").fillna(0.0).astype(float)
        )
...
def generate_risk_reward_scatter(daily_df: pd.DataFrame) -> str:
    ...
    y_values = (
        monthly_ret.values.flatten().tolist()
        if hasattr(monthly_ret, "values")
        else list(monthly_ret)
    )
