...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.trade_managers = {
            ...
            Strategies.TurnOverTiming_10: TradeManagerTurnover(
                strategy_name=Strategies.TurnOverTiming_10
            ),
            Strategies.TurnOverTiming_05: TradeManagerTurnover(
    ...
    def setup(self):
        ...
        self.console.print(
            "[bold blue]Initialising Backtest Environment...[/bold blue]"
        )
        ...
        if spy_hist.empty:
            self.console.print(
                "[yellow]WARNING: No market data for 'SPY'. Attempting fallback...[/yellow]"
            )
            fallback_dates = self.market.get_available_dates(
                str(self.start_date.date()), str(self.end_date.date())
            )
            if not fallback_dates:
                self.console.print(
                    "[bold red]CRITICAL: No market data found.[/bold red]"
                )
                return
            ...
            mask = (spy_hist.index >= self.start_date) & (
                spy_hist.index <= self.end_date
            )
            self.market_dates = sorted(spy_hist[mask].index.to_list())

        self.console.print(
            f"Found [bold green]{len(self.market_dates)}[/bold green] trading days."
        )
        self.market.preload_all_data(days=2000)

    def _get_symbol_data_slice(
        self, symbol: str, current_date: pd.Timestamp
    ) -> pd.DataFrame:
    ...
    def run(self):
        ...
        task_id = progress.add_task(
            "[cyan]Running Backtest...", total=total_days, date=self.start_date.date()
        )
    ...
    def _run_screener(self, date_obj: pd.Timestamp):
        ...
        for screener in self.screeners:
            try:
                ...
                logger.error(
                    "Screener %s Error %s: %s", screener.name, date_obj.date(), error
                )
    ...
    def _run_trade_manager_entries(self, current_date: pd.Timestamp):
        ...
        for trade in created:
            ...
            manager.check_entry(
                trade, candle, df_slice.reset_index(), self.trade_repository
            )
    ...
    def _run_trade_manager_exits(self, current_date: pd.Timestamp):
        ...
        for trade in active:
            ...
            manager.manage_active_trade(
                trade, df_slice.reset_index(), self.trade_repository
            )
